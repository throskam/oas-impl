...
it('should return extend when base is null', () => {
  const base = null
  const extend = { prop: 1 }
  expect(deepMerge(base, extend)).toBe(extend)
})

it('should keep extend when base is missing the key', () => {
  const base = { a: 1 }
  const extend = { b: 1 }
...
it('should keep extend when base has a conflicting value of the same type', () => {
  const base = { a: 1 }
  const extend = { a: 2 }
  const expected = { a: 2 }
  expect(deepMerge(base, extend)).toStrictEqual(expected)
...
it('should keep extend when type mismatch', () => {
  const base = { a: 1 }
