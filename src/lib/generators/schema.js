...
export default class SchemaGenerator {
  ...
  generate () {
    return this.recursiveGenerate(this.schema, this.option)
  }

  recursiveGenerate (schema, option) {
    ...
    if (schema.allOf) {
      return this.recursiveGenerate(schema.allOf.reduce(deepMerge, {}), option)
    }

    if (schema.anyOf) {
      return this.recursiveGenerate(schema.anyOf[0], option)
    }

    if (schema.oneOf) {
      return this.recursiveGenerate(schema.oneOf[0], option)
    }
    ...
    if (schema.type === 'object') {
      ...
      return Object.keys(schema.properties).slice(0, maxProperties).reduce((acc, key) => {
        ...
        const value = this.recursiveGenerate(schema.properties[key], option)
    ...
    if (schema.type === 'array') {
      ...
      return schemas.slice(0, maxItems).map(schema => this.recursiveGenerate(schema, option)).filter(item => item !== undefined)
    }
